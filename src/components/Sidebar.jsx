...
const Sidebar = ({ isMenuOpen }) => {
  ...
  const displaySubscriptions = (subs) => {
    return subs.slice(0, visibleSubscriptions).map((sub) => (
      <NavLink
        ...
        className={({ isActive }) =>
          `flex items-center gap-3 text-lg rounded-xl p-2 transition duration-300 ${
  ...
  return (
    <div
      className={`fixed pt-20 left-0 z-50 transition-transform duration-300 ${
        isMenuOpen ? "transform translate-x-0" : "transform -translate-x-full"
      } w-60 h-full bg-white shadow-lg`}
    >
